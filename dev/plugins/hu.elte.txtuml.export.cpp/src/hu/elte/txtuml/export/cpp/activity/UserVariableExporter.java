	...
	public void exportNewVariable(Variable var) {
		if (!variableTable.containsKey(var)) {
			if (ActivityTemplates.invalidIdentifier(var.getName())) {
				variableTable.put(var, new VariableInfo(ActivityTemplates.generatedTempVariable(genTempVariableCounter), false));
				genTempVariableCounter++;
			} else {
				variableTable.put(var, new VariableInfo(ActivityTemplates.formatUserVar(var.getName(), userVarCounter), false));
				userVarCounter++;
			}
		}
	}
