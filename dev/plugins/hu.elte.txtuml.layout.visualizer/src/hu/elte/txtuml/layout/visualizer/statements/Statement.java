...
public class Statement {
	...
	public Boolean isStrict() {
		if (_type.equals(StatementType.above) || _type.equals(StatementType.below) || _type.equals(StatementType.right)
	 ...
	 *             set.
	 */
	public Statement(StatementType t, StatementLevel level, String... params) throws InternalException {
		_type = t;
		_parameters = new ArrayList<String>();
		for (String s : params) {
			_parameters.add(s);
		...
		if (!_level.equals(StatementLevel.User))
			throw new InternalException("This statement should have group Id: " + this.toString() + "!");
		_group = null;
	}
	 ...
	 *             set.
	 */
	public Statement(StatementType t, StatementLevel level, Integer id, String... params) throws InternalException {
		_type = t;
		_parameters = new ArrayList<String>();
		for (String s : params) {
			_parameters.add(s);
	 ...
	 *             set.
	 */
	public Statement(StatementType t, StatementLevel level, ArrayList<String> params) throws InternalException {
		_type = t;
		_parameters = new ArrayList<String>();
		for (String s : params) {
			_parameters.add(s);
	 ...
	 *             set.
	 */
	public Statement(StatementType t, StatementLevel level, Integer id, ArrayList<String> params)
			throws InternalException {
		_type = t;
		_parameters = new ArrayList<String>();
		for (String s : params) {
	...
	public Statement(Statement s) {
		this._type = s._type;
		this._parameters = new ArrayList<String>(s._parameters);
	...
	public static Statement Parse(String par_input) throws UnknownStatementException {
		...
		if (parOpen == -1 || parClose == -1)
			throw new UnknownStatementException("No (not enough) parentheisis found!", input);

		try {
			type = Enum.valueOf(StatementType.class, input.substring(0, parOpen).toLowerCase());
		} catch (IllegalArgumentException e) {
			throw new UnknownStatementException("No known statement such as: " + input.substring(0, parOpen) + "!",
					input);
		}
		...
		if (!enoughParametersForType(type, params))
			throw new UnknownStatementException("Not enough / Too many parameters for type!", input);
	 ...
	 * @throws UnknownStatementException
	 *             Throws if there is no opposite of a {@link Statement}.
	 */
	public static Statement opposite(Statement s) throws InternalException, UnknownStatementException {
		return new Statement(opposite(s.getType()), s.getLevel(), s.getGroupId(), s.getParameter(1), s.getParameter(0));
	}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof Statement) {
			...
			return s1._type.equals(s2._type) && s1._parameters.equals(s2._parameters) && s1._level.equals(s2._level);
		}
	...
	public String toString() {
		...
		result += ")";
		if (!_level.equals(StatementLevel.User))
			result += "_weak(" + _level.toString() + "," + ((_group != null) ? _group.toString() : "0") + ")";
		return result;
	}

	/**
	 * Returns the complexity of this {@link Statement}.
	 * @return the complexity of this {@link Statement}.
	 */
