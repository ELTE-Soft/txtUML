...
public privileged aspect ImporterAspect {
	...
	/*Object around(Collection target):target(target) && call(Object selectOne()) && isActive()
	{
		if(target instanceof AssociationEnd)
		{
			
			return InstructionImporter.selectOne_AE((AssociationEnd)target);
		}
		else
		{
			return InstructionImporter.selectOne(target);
		}
	}*/
	...
	@SuppressAjWarnings
	void around(): call(void Action.link(Class<? extends Association>, String, ModelClass,String, ModelClass)) && isActive()
	{
	
		Class<?> assocClass=(Class<?>)(thisJoinPoint.getArgs()[0]);
		String leftPhrase=(String)(thisJoinPoint.getArgs()[1]);
		ModelClass leftObj=(ModelClass)(thisJoinPoint.getArgs()[2]);
		String rightPhrase=(String)(thisJoinPoint.getArgs()[3]);
		ModelClass rightObj=(ModelClass)(thisJoinPoint.getArgs()[4]);
		InstructionImporter.link(assocClass,leftPhrase,leftObj,rightPhrase,rightObj);	
		
	}
	@SuppressAjWarnings
	void around(): call(void Action.unlink(Class<? extends Association>, String, ModelClass,String, ModelClass)) && isActive()
	{
