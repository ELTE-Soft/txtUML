...
public class InstanceCreator {
	...
	public static <T> T createInstanceWithGivenParams(Class<T> c, Object... givenParams) {
		...
		for(Constructor<?> ctor : ctors) {
...

			{
				...
				if (i < givenParamsLen) { // one of the parameters could not be casted
					continue;
				}
				for ( ; i < len; ++i) {
					Object param = createInstanceRecursively(paramTypes[i], ancestors);
					if (param == null) {
						break;
					}
					params[i] = param;
	...
	@SuppressWarnings("unchecked")
	private static <T> T createInstanceRecursively(Class<T> c, Set<Class<?>> ancestors) {
		...
		for(Constructor<?> ctor : ctors) {
			...
			{
				int i; // we use variable 'i' after the loop to check whether it finished with 'break' command.
				for (i = 0; i < len; ++i) {
					Object param = createInstanceRecursively(paramTypes[i], ancestors);
					if (param == null) {
						break;
